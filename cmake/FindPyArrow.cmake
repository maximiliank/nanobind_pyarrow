if(NOT Python_FOUND)
  message(
    FATAL_ERROR
      "Could not find python. Make sure you called 'find_package(Python COMPONENTS Interpreter Development.Module REQUIRED)'"
  )
endif()
...
execute_process(
  ...
  OUTPUT_VARIABLE _PYARROW_CREATE_SYMLINKS_OUTPUT
  ERROR_VARIABLE _PYARROW_ERROR_VALUE)

if(_PYARROW_CREATE_SYMLINKS_SUCCESS
   AND NOT
       _PYARROW_CREATE_SYMLINKS_SUCCESS
       EQUAL
       0)
  if(_PYARROW_ERROR_VALUE MATCHES ".*No module named 'pyarrow'.*")
    message(FATAL_ERROR "pyarrow is not installed")
  endif()
  message(
    WARNING "FAILED pyarrow.create_library_symlinks(): ${_PYARROW_CREATE_SYMLINKS_SUCCESS}\n${_PYARROW_ERROR_VALUE}")
  message(
    STATUS
      "Falling back to try using known versions for arrow library. You may have to set Arrow_ADDITIONAL_VERSIONS for newer versions."
  )
...
if(_PYARROW_SEARCH_SUCCESS
   AND NOT
       _PYARROW_SEARCH_SUCCESS
       EQUAL
       0)
  message(FATAL_ERROR "FAILED: ${_PYARROW_SEARCH_SUCCESS}\n${_PYARROW_ERROR_VALUE}")
endif()

set(ARROW_INCLUDE_DIR ${PYARROW_INCLUDE_DIR})
execute_process(
  COMMAND "${Python_EXECUTABLE}" "-c" "import pyarrow as pa; print(pa.get_library_dirs());"
  RESULT_VARIABLE _PYARROW_SEARCH_SUCCESS
...
set_target_properties(pyarrow::pyarrow PROPERTIES IMPORTED_LOCATION "${ARROW_PYTHON_LIB}" IMPORTED_SONAME
                                                                                          ${PYARROW_SONAME})

add_library(nanobind_pyarrow::pyarrow INTERFACE IMPORTED)
set_property(
  TARGET nanobind_pyarrow::pyarrow
